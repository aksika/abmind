...
- **BeforeAgent** (every user turn) → runs recall on the user's prompt and injects top matches as `[abmind memory context]`. Capped at 5 memories / 2000 chars. SECRET-tier items are never returned here.
- **AfterAgent** → records the full turn (user prompt + your response) into the message store for later consolidation.
...
- **Auto-recall missed something.** The hook recall uses the raw user prompt in its original language. If the user writes a Hungarian/Spanish/etc. single word that looks like a recall test, search again in English via the MCP tool. E.g. `Martes?` → search `Tuesday Belgium`.
