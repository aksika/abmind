...
export function commitConversationCheckpoint(
  ...
  // 3. Regroup complete turns: the compacted range must end with a complete
  //    (final-assistant) turn.
  const units = groupTurnUnits(rows);
  if (units.length === 0 || !units[units.length - 1]!.complete) return { status: "rejected" };

  // 4. firstKeptMessageId must be the first actual suffix row (id > sourceEnd).
  ...
  // 5. Summary sanity: non-empty, bounded, and a real reduction of the source.
  ...
  // 6. Atomic insert + generation-guarded CAS pointer advance.
  const store = new CheckpointStore(db);
 ...
 * sessions. Throws (rolling back the migration transaction) when any session
 * has inconsistent legacy state — legacy rows are left untouched and the
 * migration fails visibly rather than pretending an empty checkpoint is valid.
 */
  ...
  for (const { chat_id: chatId } of sessions) {
    ...
    for (let i = 1; i < summaries.length; i++) {
      ...
      if (next.source_message_start <= prev.source_message_end) {
        throw new Error(`Legacy context migration refused: overlapping summary ranges in session ${chatId}`);
      }
    }
    ...
    if (!wm) {
      throw new Error(`Legacy context migration refused: missing watermark for session ${chatId}`);
    }
    if (wm.watermark_message_id !== last.source_message_end + 1) {
      throw new Error(`Legacy context migration refused: watermark inconsistent with final range in session ${chatId}`);
    }

    const firstKeptRow = db.prepare(
      "SELECT MIN(id) AS id FROM messages WHERE session_id = ? AND id >= ?",
    ).get(chatId, wm.watermark_message_id) as { id: number | null } | undefined;
    ...
    const migrateOne = db.transaction(() => {
      ...
      if (checkpointId < 0) {
        throw new Error(`Legacy context migration refused: concurrent checkpoint appeared for session ${chatId}`);
      }
